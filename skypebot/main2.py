...
class SkypeListener(SkypeEventLoop):
    ...
    def __init__(self):
        ...
        self.GetInfo_Group_ID = '19:03fb1a3c75384a6db49f4c02c4bf4414@thread.skype'
        self.GetInfo_Group  = self.chats[self.GetInfo_Group_ID]
        self.GetInfo_pattern = re.compile(r'^getinfo\s+((mir)|(vcr)|(ocr)|(atm)|(sen)|(mnc)|(dcr)|(mnv)|(shi)|(tkm))\d+\s*$')

        self.GetSms_Group_ID = '19:03fb1a3c75384a6db49f4c02c4bf4414@thread.skype'
        self.GetSms_Group = self.chats[self.GetSms_Group_ID]
    ...
    def onEvent(self, event):
        ...
        if isinstance(event, SkypeNewMessageEvent):
            ...
            if message["chat_id"] == self.GetInfo_Group_ID:
                ...
                if self.GetInfo_pattern.match(message["msg"].lower()) != None:
                    ...
                    if SearchValue != None:
                        ...
                        if rowCount == 0:
                            ...
                            self.GetInfo_Group.sendMsg('@{}: Không tìm thấy hợp đồng "{}"'.format(message["user_id"], SearchValue))
                        else:
                            ...
                            try:
                                ...
                                self.GetInfo_Group.sendMsg('@{}: Hợp đồng "{}" đã được gửi cho bạn'.format(message["user_id"], SearchValue))
                ...
                if self.GetData_pattern.match(message["msg"].lower()) != None:
                    ...
                    SearchValue = self.LOANEID_pattern.search(message["msg"].lower()).group()
                    if SearchValue != None:
                        myDBconn = pyodbc.connect(myLocalDBConn)
